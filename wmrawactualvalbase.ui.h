...
void WMRawActualValBase::ReceiveAVDataSlot( cwmActValues *ActValues )
{
	...
	if (AmplDispMode == x1_SQRT2)
	    ampl/=1.414213562;
	XnAmplDisp -> setText( QString("%1 A").arg(ampl,10,'f',5) );
	...
	*/
	...
	if (AmplDispMode == x1_SQRT2)
	    ampl/=1.414213562;
	XxAmplDisp -> setText( QString("%1 A").arg(ampl,10,'f',5) );
	...
	*/
	...
	if (AmplDispMode == x1_SQRT2)
	    ampl/=1.414213562;
	dXxAmplDisp -> setText( QString("%1 A").arg(ampl,10,'f',5) );
	...
	phi = normWinkelrad_PIPI(phi);
	phi *= radgrad;
	dXxPhaseDisp -> setText( QString("%1 %2").arg(phi,8,'f',4).arg( trUtf8("°")) );
