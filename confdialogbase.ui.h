...
void ConfDialogBase::init()
{
    ...
    RatioPrimNLineEdit->setValidator(RatioCurrentValidator);
    RatioSekNLineEdit->setValidator(RatioCurrentValidator);
    RatioPrimXLineEdit->setValidator(RatioCurrentValidator);
    RatioSekXLineEdit->setValidator(RatioCurrentValidator);
    RatioPrimECTLineEdit->setValidator(RatioCurrentValidator);
    RatioSekECTLineEdit->setValidator(RatioVoltageValidator);
...
void ConfDialogBase::accept()
{
    ...
    ratioInputOK = ratioInputOK &&  RatioPrimNLineEdit->hasAcceptableInput() 
			   ...
			   && RatioPrimECTLineEdit->hasAcceptableInput()
			   && RatioSekECTLineEdit->hasAcceptableInput();
...
void ConfDialogBase::SetRatioMenu()
{
    ...
    RatioPrimECTLineEdit->setText(baseUnitText(m_ConfData.m_ECTPrimary));
    RatioSekECTLineEdit->setText(baseUnitText(m_ConfData.m_ECTSecondary));
    ectPrim_3radioButton->setChecked(is_3(m_ConfData.m_ECTPrimary));
    ectPrim_w3radioButton->setChecked(is_w3(m_ConfData.m_ECTPrimary));
    ectSek_3radioButton->setChecked(is_3(m_ConfData.m_ECTSecondary));
    ectSek_w3radioButton->setChecked(is_w3(m_ConfData.m_ECTSecondary));
}
...
void ConfDialogBase::ApplyDataSlot() // einstellungen werden intern übernommen, die menus aktualisiert
{
	...
	case In_IxDiff:
	    ...
	    break;
	case In_ECT:
	    m_ConfData.m_ECTPrimary = genRatioText( RatioPrimECTLineEdit->text(), ectPrim_3radioButton, ectPrim_w3radioButton);
	    m_ConfData.m_ECTSecondary = genRatioText( RatioSekECTLineEdit->text(), ectSek_3radioButton, ectSek_w3radioButton);
	    break;
	}
 
    if (Mode0RadioButton->isChecked()) m_ConfData.m_nMeasMode=In_IxDiff;
    if (Mode2RadioButton->isChecked()) m_ConfData.m_nMeasMode=In_ECT;
    if (Mode3RadioButton->isChecked()) m_ConfData.m_nMeasMode=In_nConvent;
...
{
    if (ectPrim_3radioButton->isChecked())
	ectPrim_w3radioButton->setChecked(false);
}


void ConfDialogBase::ectPrim_w3radioButtonChecked()
{
    if (ectPrim_w3radioButton->isChecked())
	ectPrim_3radioButton->setChecked(false);
}


void ConfDialogBase::ectSek_3radioButtonChecked()
{
    if (ectSek_3radioButton->isChecked())
	ectSek_w3radioButton->setChecked(false);
}


void ConfDialogBase::ectSek_w3radioButtonChecked()
{
    if (ectSek_w3radioButton->isChecked())
	ectSek_3radioButton->setChecked(false);
}
